...
@unittest.skipIf(settings.KEY_VALUE_STORE == '', 'Redis is not configured in settings.')
class AdjacencyTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.district1 = max(District.objects.filter(plan=self.plan,district_id=self.district1.district_id),
                             key=lambda d: d.version)
        self.district2 = max(District.objects.filter(plan=self.plan,district_id=self.district2.district_id),
                             key=lambda d: d.version)

        # Get redis in order to set upload values for testing
        redis_settings = settings.KEY_VALUE_STORE
        self.r = redis.StrictRedis(host=redis_settings['HOST'], port=int(redis_settings['PORT']), db=15)


        base_geolevel = Geolevel.objects.get(name='smallest level')
        base_geounits = Geounit.objects.filter(geolevel=base_geolevel).order_by('portable_id')
    ...
    def testAdjacencyScores(self):
        ...
        self.assertAlmostEqual(14.7671977211, adj.result['value'], 9,
                               'Adjacency score for district was incorrect: %f' % adj.result['value'])

        # Test computing a plan's score
        adj.compute(**{'plan':self.plan, 'db':15})
        self.assertAlmostEqual(0.29712390062, adj.result['value'], 9,
