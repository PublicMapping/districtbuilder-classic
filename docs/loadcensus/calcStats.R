...
if (has_election_data) {
        ...
        q_block_VOTE_DEM_N<- myq(merged.df$VOTE_DEM_N)
        q_tract_VOTE_DEM_N<-myq(aggregate(merged.df$VOTE_DEM_N,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_DEM_N<-myq(aggregate(merged.df$VOTE_DEM_N,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
        q_block_VOTE_REP_N<- myq(merged.df$VOTE_REP_N)
        q_tract_VOTE_REP_N<-myq(aggregate(merged.df$VOTE_REP_N,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_REP_N<-myq(aggregate(merged.df$VOTE_REP_N,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
        q_block_VOTE_TOT_N<- myq(merged.df$VOTE_TOT_N)
        q_tract_VOTE_TOT_N<-myq(aggregate(merged.df$VOTE_TOT_N,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_TOT_N<-myq(aggregate(merged.df$VOTE_TOT_N,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
}
