...
if (has_election_data) {
        ...
        q_block_VOTE_DEM_NORM<- myq(merged.df$VOTE_DEM_NORM)
        q_tract_VOTE_DEM_NORM<-myq(aggregate(merged.df$VOTE_DEM_NORM,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_DEM_NORM<-myq(aggregate(merged.df$VOTE_DEM_NORM,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
        q_block_VOTE_REP_NORM<- myq(merged.df$VOTE_REP_NORM)
        q_tract_VOTE_REP_NORM<-myq(aggregate(merged.df$VOTE_REP_NORM,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_REP_NORM<-myq(aggregate(merged.df$VOTE_REP_NORM,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
        q_block_VOTE_TOT_NORM<- myq(merged.df$VOTE_TOT_NORM)
        q_tract_VOTE_TOT_NORM<-myq(aggregate(merged.df$VOTE_TOT_NORM,by=list(merged.df$TRACTCE10),sum,na.rm=T)[2])
        q_county_VOTE_TOT_NORM<-myq(aggregate(merged.df$VOTE_TOT_NORM,by=list(merged.df$COUNTYFP10),sum,na.rm=T)[2])
}
